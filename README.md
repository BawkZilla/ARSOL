...
ARSOL:
전문가 리뷰/별점 시스템 포함 → 신뢰할 수 있는 전문가와 매칭 강화.

화상 통화 종료 후 자동 리뷰 작성 기능 → 사용자 경험과 품질 관리 가능.

##### 4. 세션 기록 및 복습 기능

기존 서비스: 대부분 라이브 세션에 집중, 별도의 복습/학습 기능 제공은 제한적.

ARSOL: MediaRecorder API + Supabase DB 연동 → 세션 영상 자동 저장 & 사용자가 다시 열람 가능.
→ 조립 과정 복습, 문제 재점검, 학습 용도로 재활용 가능.
